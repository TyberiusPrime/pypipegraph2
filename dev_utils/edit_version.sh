...
files = [
	'python/pypipegraph2/__init__.py','Cargo.toml','setup.cfg','pyproject.toml'
]
...
for fn in files:
	...
	if len(matches) != 1:
		print(f"Version {old_version} found the wrong number of times ({len(matches)}) in {fn}")
		error = True
...
if error:
	...
	print("Old version {old_version} found exactly once in all files.")
