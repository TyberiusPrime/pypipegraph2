...
class TestPypipegraph2:
    ...
    def test_very_simple_chain_rerun(self, job_trace_log):
        ...
        jobA = ppg.FileGeneratingJob(
            "A", lambda of: of.write_text(f"{counter}"), allowed_non_locals=["counter"]
        )
        jobB = ppg.FileGeneratingJob(
            "B", lambda of: of.write_text("BBB" + Path("A").read_text())
        )
    ...
    def test_capturing_closures(self):
        ...
        varA = ["hello"]
        jobA = ppg.FileGeneratingJob(
            "A", lambda of: of.write_text(str(varA)), allowed_non_locals=["varA"]
        )
        ppg.run()
    ...
    def test_file_gen_when_file_existed_outside_of_graph_depending_on_cached_data_load(
        ...
        a = ppg.FileGeneratingJob(
            "A", lambda of, o=o: of.write_text("a" + o[0]), allowed_non_locals=["o"]
        )
    ...
    def test_data_loading_job_returning_none(self):
        ...
        jobA = ppg.DataLoadingJob("A", lambda self=self: self.store.append("A"))
        jobB = ppg.FileGeneratingJob(
            "B",
            lambda of: counter("b") and of.write_text(self.store[0]),
            allowed_non_locals=["self"],
        )
        jobB.depends_on(jobA)
        with pytest.raises(ppg.JobsFailed):
            ppg.run()
    ...
    def test_data_loading_job(self):
        ...
        self.store = []  # use attribute to avoid closure binding
        try:
            jobA = ppg.DataLoadingJob(
                "A", lambda self=self: self.store.append("A") or 5
            )
            jobB = ppg.FileGeneratingJob(
                "B",
                lambda of, self=self: counter("b") and of.write_text(self.store[0]),
            )
            jobB.depends_on(jobA)
            ...
            self.store.clear()
            jobA = ppg.DataLoadingJob(
                "A", lambda: self.store.append("B") or 5, allowed_non_locals=["self"]
            )
    ...
    def test_attribute_loading_job(self):
        ...
        a = TestRecv()
        jobB = ppg.FileGeneratingJob(
            "B", lambda of: counter("b") and of.write_text(a.a_), allowed_non_locals=["a"]
        )
    ...
    def test_dict_loading_job(self):
        ...
        jobB = ppg.FileGeneratingJob(
            "B",
            lambda of: counter("b") and of.write_text(a.store["a_"]),
            allowed_non_locals=["a"],
        )
        jobB.depends_on(a.job)
        ppg.run()
        assert not hasattr(a, "a_")
    ...
    def test_actually_multithreading(self):
        ...
        def inner(of):
            def inner2(of=of, barrier=barrier):
                c = barrier.wait()
                write(of, str(c))
    ...
    def test_multi_file_generating_job_with_dict_file_definition(self):
        ...
        b = ppg.FileGeneratingJob(
            "c", lambda of, a=a: of.write_text(a["a"].read_text())
        )
        b.depends_on(a["a"])
        ppg.run()
        with pytest.raises(ValueError):
    ...
    def test_failing_job_but_required_again_after_job_generating_job(
        ...
        c = ppg.JobGeneratingJob("c", c, allowed_non_locals=['a'])
        with pytest.raises(ppg.JobsFailed):
            ppg.run()
        assert read("a") == "1"
        assert read("c") == "1"
